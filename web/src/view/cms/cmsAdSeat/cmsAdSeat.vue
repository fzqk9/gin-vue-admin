    ...
    >
      <el-table-column type="selection" width="55" />      
       <el-table-column label="ID" min-width="60" prop="ID" sortable="custom" />
          <!--name  BeQuickEdit -->  
        <el-table-column label="位置名称" prop="name" width="120"   >
        <template #default="scope">
          ...
          <!--width  BeQuickEdit -->  
        <el-table-column label="广告位宽度" prop="width" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.width"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('width',scope.row.ID,scope.row.width,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.width}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
          <!--height  BeQuickEdit -->  
        <el-table-column label="广告位高度" prop="height" width="120"   sortable="custom" >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.height"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('height',scope.row.ID,scope.row.height,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.height}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column> 
      <!--desc BeHide -->
          <!--style  BeQuickEdit -->  
        <el-table-column label="模板" prop="style" width="120"   >
        <template #default="scope">
            <el-popover trigger="click" placement="top"  width = "280">  
            <el-row :gutter="10">
              <el-col :span="16">  <el-input type="textarea" autosize placeholder="请输入内容" v-model="scope.row.style"></el-input></el-col>
              <el-col :span="4"> <el-button size="small" type="primary" icon="el-icon-edit" class="table-button" @click="quickEdit_do('style',scope.row.ID,scope.row.style,scope)">保存</el-button> </el-col> 
            </el-row>  
              <template #reference>
                <div  class="quickEditTxt"  > {{scope.row.style}} </div>
              </template>
            </el-popover>
        </template>
          </el-table-column>
...
export default {
  ...
  methods: { 
    ...
    async onDelete() {
  	...
  	async doDelete(ids) { 
     const res = await deleteCmsAdSeatByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功'
   ...
   async goEditForm(id) { 
	  if (this.beNewWindow) {
		  if (id >0) {
			this.$router.push({ name:'cmsAdSeatForm', params:{id:id}})
		  } else {
			 this.$router.push({ name:'cmsAdSeatForm',params:{id:id}})
		  }
	  }else
