	...
	<el-dialog v-model="showDialog" width="80%" title="媒体库">
		<template #title>
            ...
            <el-form-item label="用户id">
					...
					<el-form-item label="模块名" prop="module">
						<el-select @change="module_change" @clear="module_clear" v-model="searchInfo.module"
							placeholder="请选择" clearable>
							<el-option v-for="(item,key) in moduleOptions" :key="key" :label="item.label"
								:value="item.value"></el-option>
						</el-select>
					</el-form-item>

					<el-form-item label="媒体类型" prop="mediaType">
						<el-select @change="mediaType_change" @clear="mediaType_clear" v-model="searchInfo.mediaType"
							placeholder="请选择" clearable>
					...
					<el-form-item>
						...
						<el-upload :action="`${path}/commFile/upload`" :data="uploadData"
							:headers="{ 'x-token': token }" :show-file-list="false" :on-success="handleImageSuccess"
		...
		<template #footer>
			<el-pagination layout="total, sizes, prev, pager, next, jumper" :current-page="page" :page-size="pageSize"
				:page-sizes="[10,20, 50, 100]" :style="{float:'right',padding:'20px'}" :total="total"
